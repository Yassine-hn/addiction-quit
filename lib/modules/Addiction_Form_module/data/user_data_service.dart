...
class UserDataService {
  ...
  /// Only allows deleting the current active user
  static Future<bool> deleteCurrentUser() async {
    try {
      ...
      // Delete all addictions for this user
      await db.delete(
        'addictions',
        where: 'user_id = ?',
        whereArgs: [userId],
      ...
      // Delete all daily surveys for this user
      await db.delete(
        'daily_surveys',
        where: 'user_id = ?',
        whereArgs: [userId],
      ...
      // Delete the user
      await UsersTable.delete(db, userId);
